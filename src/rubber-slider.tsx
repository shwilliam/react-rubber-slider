...
import { getDragTarget, getEventCoords, getNormalizedOffset, getPosFromValue, getSteppedValue, getValueFromPos } from './utils';
import { IRubberSliderProps, TSliderPos } from './rubber-slider.d';

export const RubberSlider: React.FC<IRubberSliderProps> = ({ id = 'rubber-slider', name, className = '', value = 0, onChange = (value: number): void => {}, width = 200, height = 100, max = 100, min = 0, step = 1, easeFunction = easeElastic, easeDuration = 700, onDragStart = (position: TSliderPos): void => {}, onDrag = (position: TSliderPos): void => {}, onDragEnd = (position: TSliderPos): void => {}, style = {} }) => {
  const points = [
    [0, height / 2],
  ...
  const draw = () => {
    const svg = select(`#${id}-container`)
      ...
      .call(drag().subject(getDragTarget).on('start', handleDragStart).on('drag', handleDrag).on('end', handleDragEnd) as any);
  ...
  return (
    <SliderInput name={name} value={value} onChange={handleChange} min={min} max={max} step={step} className={`rubber-slider-input ${className}`} style={{ ...style, width }}>
