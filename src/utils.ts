...
export const getSteppedValue = (value: number, step: number) => value - (value % step)
...
export const getValueFromPos = (pos: number, posMax: number, min: number, max: number) =>
	(pos / posMax) * (max - min) + min

export const getPosFromValue = (value: number, maxPos: number, min: number, max: number) =>
